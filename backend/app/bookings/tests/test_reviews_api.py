...
class MyReviewViewSetTest(APITestCase):
    def setUp(self):
        self.list_url = reverse('my-review-list')
    ...
    def _get_detail_url(self, review_id):
        return reverse('my-review-detail', kwargs={'pk': review_id})

    def _get_archive_url(self, review_id):
        return reverse('my-review-archive', kwargs={'pk': review_id})

    def _get_submit_url(self, review_id):
        return reverse('my-review-submit', kwargs={'pk': review_id})

    def test_unauthenticated_cannot_list_reviews(self):
        response = self.client.get(self.list_url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_non_guest_cannot_list_reviews(self):
    ...
    def test_default_ordering(self):
        self.client.force_authenticate(self.guest_user)
        response = self.client.get(self.list_url)
        created_at_list = [datetime.fromisoformat(r['created_at']) for r in response.data]
